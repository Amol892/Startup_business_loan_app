...
function CustomerCard({users}) {
        ...
        async function fetchDocuments(data){
            await axios.get(`http://localhost:8000/document_verify/documents/${data}/`,
                ...
                setMessage(response.data.message)
                navigate('/lsodocuments',{state:{user : Doc_data}})
            }).catch(error=>{
                setError("Data is not available")
                console.log(error.response.data)
        ...
        {   
            ...
            users.map(obj=>{
                ...
                if (obj.status==='Pending') {
                        ...
                        check_document = <button style={{padding:10,fontSize:15,borderRadius:10,marginLeft:15,backgroundColor:'aqua'}} onClick={(e)=>fetchDocuments(e.target.value)} value={obj.id} >Check Documents</button>
                ...
                return(
                    ...
                    <div style={{backgroundColor:'white',borderRadius:20,display:'flex',width:700,marginRight:30,marginTop:10}}>
                        ...
                        <h5>Loan Status    : <NavLink className={status_color}><b>{obj.status}</b></NavLink>{check_document}</h5>
                        </div>
