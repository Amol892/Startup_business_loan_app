...
function LSODocumentCard() {
    ...
    function lsoapprovedocuments(data){
      console.log(data)
      data={'s1':data}
      console.log(user.application.id)
      axios.patch(`http://localhost:8000/loan_sanctioning/documentstatus/${user.application.id}/`,data,
        {headers:{'Content-Type':'multipart/form-data',
                  "Authorization":'Bearer' + " " + access
                  }}
                  ).then(response=>{
                    navigate('/lsoDashboard')
                  }).catch(error=>{
                    console.log(error)
                  })
  ...
  return (
    <>
        ...
        <center><div style={{color:'black',backgroundColor:'khaki',padding:20}}><h2>Applicants Documents</h2></div></center>
              ...
              <div className='col-2'>
              <button style={{fontSize:20,fontWeight:500,padding:50,marginTop:50,width:200,height:150}} type="button" class="btn btn-success" value='approve' onClick={(e)=>{lsoapprovedocuments(e.target.value)}}>Approve</button><br/><br/>
              <button style={{fontSize:20,fontWeight:500,padding:50,marginTop:50,width:200,height:150}} type="button" class="btn btn-danger"  value='rejected' onClick={(e)=>{lsoapprovedocuments(e.target.value)}}>Reject</button>
...
export default LSODocumentCard